...
public class WallHandler implements Serializable {
	...
	public void load() {
		Map<String, String> map = (Map<String, String>) FacesContext
	...
	public List<UserLogMessage> getMessagesByUser() {
		...
		if (this.id > 0) {
			Representation wr = ClientHandler.getNormalResource("/wall/"
					+ this.id);
			JsonRepresentation jsonRep;
			try {
				jsonRep = new JsonRepresentation(wr);
				messages = Converter.fromJsonToList(jsonRep.getText(),
						new TypeToken<List<UserLogMessage>>() {
						}.getType());
	...
	public void save() {
		...
		WallResource wr = ClientHandler.getObjectResource("/wall/"
				+ getTokenSession().getProfile().getUserProfileId(),
				WallResource.class);
